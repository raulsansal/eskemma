...
export default function ExploracionPage() {
  ...
  const [activeTab, setActiveTab] = useState<'cuantitativo' | 'cualitativo' | 'contexto' | 'fuentes'>('cuantitativo');
  ...
  const handleCompletePhase = async () => {
    ...
    const hasQuantitative = data?.quantitativeInsights && data.quantitativeInsights.length > 0;
    const hasQualitative = data?.qualitativeInsights && data.qualitativeInsights.length > 0;

    if (!hasQuantitative && !hasQualitative) {
      alert('Por favor agrega al menos un insight cuantitativo o cualitativo antes de continuar.');
      return;
    }
  ...
  const updateElectoralContext = (updates: Partial<ElectoralContext>) => {
    ...
    // Filtrar valores undefined antes de hacer merge (Firestore no acepta undefined)
    const filteredUpdates = Object.fromEntries(
      Object.entries(updates).filter(([_, value]) => value !== undefined)
    );
    
    updateData({ electoralContext: { ...current, ...filteredUpdates } });
  };
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm text-blue-800">
          ...
          <span className="font-semibold">Síntesis Legislativa</span> para obtener datos.
        </p>
      </div>
      ...
      <div className="border-b border-gray-eske-20 mb-6">
        <nav className="flex gap-1 overflow-x-auto">
          {[
            { id: 'cuantitativo', label: 'Datos Cuantitativos', count: data?.quantitativeInsights?.length || 0 },
            { id: 'cualitativo', label: 'Insights Cualitativos', count: data?.qualitativeInsights?.length || 0 },
            { id: 'contexto', label: 'Contexto Electoral' },
            { id: 'fuentes', label: 'Fuentes', count: data?.sources?.length || 0 },
          ].map((tab) => (
            <button
              key={tab.id}
...
function TabCuantitativo({ insights, onAdd, onRemove }: TabCuantitativoProps) {
  ...
  const handleSubmit = () => {
    ...
    // Construir objeto solo con campos definidos (Firestore no acepta undefined)
    const insight: QuantitativeInsight = {
    ...
    if (form.date) insight.date = form.date;
    if (form.trend) insight.trend = form.trend;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-eske-50">Métricas, estadísticas y datos numéricos relevantes</p>
        </div>
        {!showForm && (
          <button
        ...
        <div className="bg-gray-eske-10 rounded-lg p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={form.trend || ''}
                onChange={(e) => setForm({ ...form, trend: e.target.value as QuantitativeInsight['trend'] })}
      ...
      {insights.length > 0 ? (
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-medium text-gray-eske-80">{insight.metric}</span>
                  {insight.trend && (
                    <span className={`text-sm ${
                      insight.trend === 'up' ? 'text-green-600' :
                      insight.trend === 'down' ? 'text-red-600' : 'text-gray-eske-50'
                    }`}>
                      {insight.trend === 'up' ? '↑' : insight.trend === 'down' ? '↓' : '→'}
                    </span>
                  )}
                </div>
                <p className="text-lg font-semibold text-bluegreen-eske">{insight.value}</p>
        ...
        <div className="text-center py-8 text-gray-eske-50">
          ...
          <p className="text-sm mt-1">Agrega métricas como encuestas, estadísticas, indicadores, etc.</p>
        </div>
      )}
    </div>
  );
...
function TabCualitativo({ insights, onAdd, onRemove }: TabCualitativoProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-medium text-gray-eske-80">Insights Cualitativos</h3>
          <p className="text-sm text-gray-eske-50">Opiniones, percepciones y narrativas del entorno</p>
        </div>
        {!showForm && (
          <button
            onClick={() => setShowForm(true)}
        ...
        <div className="bg-gray-eske-10 rounded-lg p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                value={form.category || ''}
                onChange={(e) => setForm({ ...form, category: e.target.value as QualitativeInsight['category'] })}
                className="w-full px-3 py-2 border border-gray-eske-30 rounded-lg focus:ring-2 focus:ring-bluegreen-eske focus:border-transparent bg-white-eske"
              >
                <option value="">Seleccionar...</option>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, relevance: e.target.value as QualitativeInsight['relevance'] })}
                className="w-full px-3 py-2 border border-gray-eske-30 rounded-lg focus:ring-2 focus:ring-bluegreen-eske focus:border-transparent bg-white-eske"
              >
      ...
      {insights.length > 0 ? (
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={`px-2 py-0.5 text-xs font-medium rounded ${
                    insight.relevance === 'high' ? 'bg-red-100 text-red-700' :
                    insight.relevance === 'low' ? 'bg-gray-100 text-gray-600' :
                ...
                <p className="text-xs text-gray-eske-50 mt-2">Fuente: {insight.source}</p>
              </div>
              <button
                onClick={() => onRemove(index)}
                className="p-1.5 text-gray-eske-40 hover:text-red-500 transition-colors"
        ...
        <div className="text-center py-8 text-gray-eske-50">
          ...
          <p className="text-sm mt-1">Agrega opiniones, percepciones y narrativas relevantes</p>
        </div>
      )}
    </div>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              // Solo actualizar si hay un valor (evitar strings vacíos)
              if (value) {
                onChange({ electionDate: value });
              }
            }}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              const value = e.target.value;
              // Solo actualizar si hay un valor numérico válido
              if (value === '') {
                // Si el campo está vacío, no enviamos nada (evitar undefined)
                // El campo simplemente no se actualiza
              } else {
                const numValue = parseInt(value);
                if (!isNaN(numValue)) {
                  onChange({ expectedTurnout: numValue });
                }
              }
            }}
            placeholder="Ej: 65"
        ...
        {context?.keyIssues && context.keyIssues.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {context.keyIssues.map((issue, index) => (
              ...
              >
                {issue}
                <button onClick={() => removeIssue(index)} className="hover:text-bluegreen-eske/70">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
                </button>
...
function TabFuentes({ sources, onAdd, onRemove }: TabFuentesProps) {
  ...
  const handleSubmit = () => {
    ...
    // Solo agregar campos opcionales si tienen valor
    if (form.url?.trim()) source.url = form.url.trim();
    if (form.date) source.date = form.date;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-eske-50">Registra las fuentes utilizadas para la exploración</p>
        </div>
        {!showForm && (
          <button
            onClick={() => setShowForm(true)}
        ...
        <div className="bg-gray-eske-10 rounded-lg p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={form.type || ''}
                onChange={(e) => setForm({ ...form, type: e.target.value as DataSource['type'] })}
                className="w-full px-3 py-2 border border-gray-eske-30 rounded-lg focus:ring-2 focus:ring-bluegreen-eske focus:border-transparent bg-white-eske"
              >
                <option value="">Seleccionar...</option>
      ...
      {sources.length > 0 ? (
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-gray-eske-80 truncate">{source.name}</p>
                  {source.url && (
                    <a
                      href={source.url}
                      target="_blank"
                ...
                {source.date && (
                  <span className="text-xs text-gray-eske-50 shrink-0">{source.date}</span>
                )}
              </div>
              <button
                onClick={() => onRemove(index)}
        ...
        <div className="text-center py-8 text-gray-eske-50">
          ...
          <p className="text-sm mt-1">Agrega las fuentes de donde obtuviste los datos</p>
        </div>
      )}
    </div>
  );
